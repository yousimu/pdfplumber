...
from _utils import logger, WIKI_BASE_PATH, get_safe_title, PROXIES

MD_DIR = WIKI_BASE_PATH
COVERS_DIR = WIKI_BASE_PATH + "/covers"

def download_and_save_cover_image(meta, img_filename):
    """
    下载并保存图书封面图片
    """
    try:
...
def build_md(meta):
    ...
    md_path = os.path.join(MD_DIR, f"{safetitle}.md")
    
    img_path = ""
    # 处理封面图片，避免上次下载失败，总是尝试
    if "imageLink" in meta and meta["imageLink"]:
        img_filename = f"{meta["id"]}.jpg"
        img_path = download_and_save_cover_image(meta, img_filename)
        logger.debug(f"封面图片已保存: {img_path}")
    
    # 检查 MD 文件是否已经存在
    if os.path.exists(md_path):
        logger.debug(f"Markdown 文件已存在:[ {os.path.basename(md_path)} ]")
        return md_path
    
    logger.debug(f"正在生成 Markdown 文件:[ {os.path.basename(md_path)} ]")
    ...
    if img_path:
        md_content.append(f"![封面](covers/{img_filename})")
        md_content.append("")
    else: # 如果没有封面图片，则使用占位符
...
def get_toc_from_xml(meta:Dict[str,str]) -> str:
    ...
    if not prefix:
        logger.debug(f"[warn] 无法获取文件 {filename} 的前缀")
        return ""
    ...
    if not os.path.exists(xml_file_path):
        logger.debug(f"[warn] 目录XML文件不存在: {xml_file_path}")
        return ""
    ...
    try:
        ...
        logger.debug(f"[warn] 无法读取目录XML文件 {xml_file_path}: {e}")
        return ""
    ...
    try:
        ...
        logger.debug(f"[warn] 解析目录XML时出错: {e}")
        return ""
